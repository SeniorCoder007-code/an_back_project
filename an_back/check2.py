...
class Checker:
	...
	def __init__(self):
		...
		self.email = '' # here you put the support email
		self.key = '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'
	...
	def report_error(self,reason,message):
		...
		message_form = Mail(
    from_email='checker@anonymix.com',
    to_emails=self.email,
    subject=subject,
    html_content=full_message)
		sg = SendGridAPIClient(self.key)
		resp = sg.send(message_form)
		...
		try:
			...
			if data:
				...
				if response:
					...
					self.log('response failed (passcode is False)')
			else:
				pass
		except Exception as e:
			print('error')
